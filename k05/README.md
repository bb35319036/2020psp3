...
118 スタックに出発点を格納
120 スタックが空になるまで繰り返す
124-133 スタックから取り出した場所が行ったことある場所ならvisitedを１にし、その場所を出力する。その後隣接行列を参考にしその場所から行ける場所（表の数値が0より大きい場所）をスタックに格納
...
212 キューに出発点を格納
214 スタックが空になるまで繰り返す
216-227 キューから取り出した場所が行ったことがあればvisitedを1にしその場所を出力し、その後隣接行列を参考にし行ける場所をキューに入れる

## 出力結果

```
PS C:\Users\admin\Desktop\プログラミング演習Ⅲ\2020psp3\k05> gcc -o k05 k05.c -w
PS C:\Users\admin\Desktop\プログラミング演習Ⅲ\2020psp3\k05> ./k05
赤迫
長崎大学前
長崎駅前
市民会館
