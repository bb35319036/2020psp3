...
78 先頭を指すポインタの初期化
82 リストを作るメモリの確保
84 pNodeのcityの部分に表から持ってきた新しい値(newCity)を 格納
85 pNodeのポインタが次のポインタを指すようにする。
86 topが新しい要素を指すようにする。
89　その時のtopの値を返す

118 リストを作る構造体を宣言
119 すでに作ったリストを使えるようにpListの先頭ポインタをpNodeに代入
120　pNodeの要素がなくなるまで繰り返す。
124 入力したIDとリストのIDが一致した場合リストの中身を代入
125　その時のiの値をresultに代入。
132　pNodeがpNodeのpNextを指すようにする。
134　繰り返した回数が分かるようにiの値を更新する。
...
```
PS C:\Users\admin\Desktop\プログラミング演習Ⅲ\2020psp3\k02> ./k02
